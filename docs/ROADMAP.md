...
| Gap | Details | Relevant Files | Priority |
|-----|---------|----------------|----------|
| GC configuration hidden | Runtime supports multiple strategies/env vars (`src/runtime/memory/config.rs:1-92`), but the CLI and docs never surface `OTTER_GC_*`. Add CLI flags (`otter run --gc-strategy mark-sweep`), pass through to the runtime, and document expected behavior. | CLI, runtime config, docs | High |
| Arena/root APIs undocumented | `otter_arena_*` helpers and root registration exist (`src/runtime/stdlib/gc.rs`) but lack user guidance. Add a GC guide explaining how to allocate, reset, and avoid dangling pointers from Otter code and FFI. | Docs | Medium |
